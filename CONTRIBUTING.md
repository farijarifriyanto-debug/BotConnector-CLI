...
By default, `bun dev` runs OpenCode in the `packages/botconnector` directory. To run it against a different directory or repository:
...
```bash
./packages/botconnector/script/build.ts --single
```

Then run it with:

```bash
./packages/botconnector/dist/opencode-<platform>/bin/opencode
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

- Core pieces:
  - `packages/botconnector`: OpenCode core business logic & server.
  - `packages/botconnector/src/cli/cmd/tui/`: The TUI code, written in SolidJS with [opentui](https://github.com/sst/opentui)
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/botconnector/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/botconnector ./src/index.ts serve --port 4096`,
    then attach TUI with `opencode attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/botconnector ./src/index.ts`
