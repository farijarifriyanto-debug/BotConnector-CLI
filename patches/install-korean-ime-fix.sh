...
PROMPT_FILE="$OPENCODE_SRC/packages/botconnector/src/cli/cmd/tui/component/prompt/index.tsx"
...
cd "$OPENCODE_SRC/packages/botconnector"
...
BUILT_BINARY="$OPENCODE_SRC/packages/botconnector/dist/opencode-${PLATFORM}-${ARCH}/bin/opencode"

if [ ! -f "$BUILT_BINARY" ]; then
  BUILT_BINARY=$(find "$OPENCODE_SRC/packages/botconnector/dist" -name "opencode" -type f -executable 2>/dev/null | head -1)
fi
...
if [ -f "$BUILT_BINARY" ]; then
  ...
  echo "  cd $OPENCODE_SRC/packages/botconnector && bun run build --single"
  exit 1
fi
