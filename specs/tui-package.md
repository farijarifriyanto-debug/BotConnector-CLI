...
`packages/botconnector/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @botconnector/tui
```

Target dependency graph:

```text
packages/botconnector ---\
                      > @botconnector/tui -> @opencode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/botconnector`,
`packages/cli`, or `@botconnector/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/botconnector`.
- Do not replace private `packages/botconnector` imports with `@botconnector/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@botconnector/tui`.

## Ownership Boundary

### `@botconnector/tui` Owns
...
```text
packages/botconnector/src/cli/cmd/tui
```

Its private dependency on `packages/botconnector` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/botconnector/src/*`.
...
Status: Completed. The private `@botconnector/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@botconnector/tui`.
...
primitives now live in `@botconnector/tui`. The legacy host consumes explicit
...
and build inputs through `@botconnector/tui/runtime`. Movable app, component,
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/botconnector` adapters.
...
`@botconnector/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/botconnector` initially.
...
canonical in `@botconnector/tui`. Configured references resolve through the new
...
`@botconnector/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/botconnector`.
...
- The package has no imports from `packages/botconnector`, `packages/cli`, or
  `@botconnector/core`.
...
public `@botconnector/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/botconnector`.
- Keep the legacy embedded worker and server startup in `packages/botconnector`.
- Change those adapters to load config, create transport inputs, and call the
  public `@botconnector/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/botconnector`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@botconnector/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@botconnector/core`, or either
  executable package.
...
cd packages/botconnector && bun typecheck
...
rg '@botconnector/core|packages/botconnector|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/botconnector/src packages/botconnector/test
```
...
- Build the current-platform `packages/botconnector` binary.
