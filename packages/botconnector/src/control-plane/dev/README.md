This is a plugin to simulate a remote environment locally. Add this to `.botconnector/botconnector.jsonc`:

```json
  "plugin": ["../packages/botconnector/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate upstream engine server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/botconnector/script/run-workspace-server
```

With the plugin install, you can now run upstream engine and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
