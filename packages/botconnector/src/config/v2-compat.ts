...
import { NonNegativeInt, PositiveInt } from "@botconnector/core/schema"
import { ConfigAttachmentV1 } from "@botconnector/core/v1/config/attachment"
import { ConfigLSPV1 } from "@botconnector/core/v1/config/lsp"
import { InvalidError } from "@botconnector/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'These V2 permissions are not supported by this BotConnector runtime. Use the supported "permission" rules.',
      })),
    })
