...
import { Pty } from "@botconnector/core/pty"
import { PtyProtocol } from "@botconnector/core/pty/protocol"
import { PtyID } from "@botconnector/core/pty/schema"
...
import { CorsConfig, isAllowedRequestOrigin, type CorsOptions } from "@botconnector/server/cors"
import {
...
export const ptyHandlers = HttpApiBuilder.group(InstanceHttpApi, "pty", (handlers) =>
  Effect.gen(function* () {
    ...
    const connectToken = Effect.fn("PtyHttpApi.connectToken")(function* (ctx: { params: { ptyID: PtyID } }) {
      ...
      const token = request.headers[PTY_CONNECT_TOKEN_HEADER] ?? request.headers[PTY_CONNECT_TOKEN_HEADER_LEGACY]
      if (token !== PTY_CONNECT_TOKEN_HEADER_VALUE || !validOrigin(request, cors))
