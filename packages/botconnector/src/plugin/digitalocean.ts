...
import { OauthCallbackPage } from "@botconnector/core/oauth/page"
...
async function listRouters(
  ...
  const res = await fetch(`${DO_GENAI_API}/models/routers`, {
    headers: {
      ...
      "User-Agent": `botconnector/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              url,
              instructions:
                "Sign in to DigitalOcean in your browser. BotConnector will use your DigitalOcean API token directly for inference and load your Inference Routers. Re-run /connect to refresh routers later.",
