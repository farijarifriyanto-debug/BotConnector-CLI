...
import { errorData, errorMessage } from "@botconnector/tui/util/error"
import { isRecord } from "@botconnector/tui/util/record"
...
import { hasTheme, upsertTheme } from "@botconnector/tui/context/theme"
import { Global } from "@botconnector/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@botconnector/core/util/flock"
import { Flag } from "@botconnector/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@botconnector/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@botconnector/core/v1/config/plugin"
import { createCommandShim } from "@botconnector/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@botconnector/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".botconnector"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".botconnector", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".botconnector", "tui.json"),
  }
}
