...
export type PatchDeps = {
  ...
  files: (dir: string, name: "botconnector" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".botconnector")
}

function patchName(kind: Kind): "botconnector" | "tui" {
  if (kind === "server") return "botconnector"
  return "tui"
}
