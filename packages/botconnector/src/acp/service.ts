...
export class Service extends Context.Service<Service, Interface>()("@botconnector/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `botconnector auth login` in the terminal",
      name: "Login with BotConnector",
      id: AuthMethodID,
    }
    ...
    if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
      authMethod._meta = {
        "terminal-auth": {
          command: "botconnector",
          args: ["auth", "login"],
          label: "BotConnector Login",
        },
      }
    }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "BotConnector prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "BotConnector service failure", service })
}
