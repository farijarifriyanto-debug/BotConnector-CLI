import type { NamedError } from "@botconnector/core/util/error"
...
export const GO_UPSELL_URL = "https://app.botconnector.id"
...
export function retryable(error: Err, provider: string) {
  ...
  if (SessionV1.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: "Open BotConnector to review your available models, balance, or plan.",
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = "https://app.botconnector.id"
      return {
