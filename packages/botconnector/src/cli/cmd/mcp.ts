...
import { ConfigMCPV1 } from "@botconnector/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in botconnector.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .botconnector/ subdirectory too)
  const candidates = [path.join(baseDir, "botconnector.json"), path.join(baseDir, "botconnector.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".botconnector", "botconnector.json"), path.join(baseDir, ".botconnector", "botconnector.jsonc"))
  }
  ...
  // Default to botconnector.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., botconnector x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "botconnector-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
