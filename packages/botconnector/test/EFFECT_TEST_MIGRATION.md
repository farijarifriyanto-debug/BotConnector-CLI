...
9. Run the focused test file and `bun typecheck` from `packages/botconnector`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/botconnector/test
```
