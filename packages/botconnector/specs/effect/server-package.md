...
- The server still lives in `packages/botconnector`.
...
Until enough shared service code lives outside `packages/botconnector`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/botconnector`

It should not import `packages/botconnector` services while `packages/botconnector`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/botconnector` implementation details.
