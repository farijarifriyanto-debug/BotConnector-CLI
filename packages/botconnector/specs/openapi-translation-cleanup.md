...
Trim `packages/botconnector/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/botconnector/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/botconnector`.
- `[x]` `bun typecheck` from `packages/botconnector`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/botconnector/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/botconnector`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/botconnector`.
...
- Edit `packages/botconnector/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/botconnector`.
...
- Check source schemas in files like `packages/botconnector/src/session/schema.ts`, `packages/botconnector/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/botconnector/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/botconnector/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
