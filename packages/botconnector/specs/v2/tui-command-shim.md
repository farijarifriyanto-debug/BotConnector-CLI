...
Delete `packages/botconnector/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/botconnector/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/botconnector/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/botconnector`
- TUI plugin loader tests in `packages/botconnector` if runtime plugin API wiring changed
