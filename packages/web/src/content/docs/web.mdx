---
title: Web
description: Using BotConnector in your browser.
---

BotConnector can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![BotConnector Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens BotConnector in your default browser.
...
By default, BotConnector picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make BotConnector accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, BotConnector will display both local and network addresses:
...
Once started, the web interface provides access to your BotConnector sessions.

### Sessions

View and manage your sessions from the homepage. You can see active sessions and start new ones.

![BotConnector Web - Active Session](../../assets/web/web-homepage-active-session.png)

### Server Status

Click "See Servers" to view connected servers and their status.

![BotConnector Web - See Servers](../../assets/web/web-homepage-see-servers.png)
...
You can also configure server settings in your `botconnector.json` config file:
