...
You can also have global rules in a `~/.config/botconnector/AGENTS.md` file. This gets applied across all opencode sessions.

Since this isn't committed to Git or shared with your team, we recommend using this to specify any personal rules that the LLM should follow.

### Claude Code Compatibility

For users migrating from Claude Code, BotConnector supports Claude Code's file conventions as fallbacks:

- **Project rules**: `CLAUDE.md` in your project directory (used if no `AGENTS.md` exists)
- **Global rules**: `~/.claude/CLAUDE.md` (used if no `~/.config/botconnector/AGENTS.md` exists)
...
2. **Global file** at `~/.config/botconnector/AGENTS.md`
3. **Claude Code file** at `~/.claude/CLAUDE.md` (unless disabled)

The first matching file wins in each category. For example, if you have both `AGENTS.md` and `CLAUDE.md`, only `AGENTS.md` is used. Similarly, `~/.config/botconnector/AGENTS.md` takes precedence over `~/.claude/CLAUDE.md`.

---

## Custom Instructions

You can specify custom instruction files in your `botconnector.json` or the global `~/.config/botconnector/botconnector.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
### Using botconnector.json

The recommended approach is to use the `instructions` field in `botconnector.json`:
...
:::tip
For monorepos or projects with shared standards, using `botconnector.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
