...
There are two types of agents in BotConnector; primary agents and subagents.
...
BotConnector comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
BotConnector comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

BotConnector comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into BotConnector's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `botconnector.json` config file:
...
- Global: `~/.config/botconnector/agents/`
- Per-project: `.botconnector/agents/`

```markdown title="~/.config/botconnector/agents/review.md"
---
...
If no temperature is specified, BotConnector uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global BotConnector config and the project specific config.
...
The model ID in your BotConnector config uses the format `provider/model-id`. For example, if you're using [BotConnector Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="botconnector.json" {3-6,9-12}
{
  "tools": {
    "write": true,
    "bash": true
...
```json title="botconnector.json" {3-5,8-10}
{
  "permission": {
    "edit": "deny"
  },
...
```json title="botconnector.json" {7}
{
  "agent": {
    "build": {
      "permission": {
...
```json title="botconnector.json" {8}
{
  "agent": {
    "build": {
      "permission": {
...
```json title="botconnector.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
```markdown title="~/.config/botconnector/agents/docs-writer.md"
---
...
```markdown title="~/.config/botconnector/agents/security-auditor.md"
---
