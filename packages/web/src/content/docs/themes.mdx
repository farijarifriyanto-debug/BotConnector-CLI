...
With BotConnector you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, BotConnector uses our own `opencode` theme.
...
BotConnector comes with several built-in themes.
...
- Want BotConnector to match their terminal's appearance
...
BotConnector supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/botconnector/themes/*.json` or `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.botconnector/themes/*.json`
4. **Current working directory** - Defined in `./.botconnector/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/botconnector/themes
vim ~/.config/botconnector/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .botconnector/themes
vim .botconnector/themes/my-theme.json
```
