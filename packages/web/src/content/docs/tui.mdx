---
title: TUI
description: Using the BotConnector terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

BotConnector provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running BotConnector starts the TUI for the current directory.
...
When using the BotConnector TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to BotConnector. Allows you to select from available providers and add their API keys.
...
Exit BotConnector. _Aliases_: `/quit`, `/q`
...
This is separate from `botconnector.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long BotConnector waits after the leader key. Defaults to `2000`.
