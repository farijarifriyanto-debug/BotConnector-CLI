---
title: CLI
description: BotConnector CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The BotConnector CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with BotConnector programmatically.
...
Start the BotConnector terminal user interface.
...
The BotConnector CLI also has the following commands.

---

### agent

Manage agents for BotConnector.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.botconnector/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running BotConnector backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote BotConnector backend. For example:
...
BotConnector is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When BotConnector starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in BotConnector.
...
Start a headless BotConnector server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
List all BotConnector sessions.
...
Delete an BotConnector session.
...
Show token usage and cost statistics for your BotConnector sessions.
...
Import session data from a JSON file or BotConnector share URL.

```bash
opencode import <file>
```

You can import from a local file or an BotConnector share URL.
...
Start a headless BotConnector server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access BotConnector through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run BotConnector.
...
Uninstall BotConnector and remove all related files.
...
BotConnector can be configured using environment variables.

| Variable                              | Type    | Description                                       |
| ------------------------------------- | ------- | ------------------------------------------------- |
| `OPENCODE_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `OPENCODE_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `BOTCONNECTOR_CONFIG`                     | string  | Path to config file                               |
| `OPENCODE_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `BOTCONNECTOR_CONFIG_DIR`                 | string  | Path to config directory                          |
| `BOTCONNECTOR_CONFIG_CONTENT`             | string  | Inline json config content                        |
