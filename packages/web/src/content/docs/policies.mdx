---
title: Policies
description: Control which configured resources BotConnector may use.
---

Policies control whether BotConnector may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `botconnector.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether BotConnector may use a resource such as an LLM provider.
...
BotConnector currently supports one policy action:
