...
BotConnector Go is a low cost **$10/month subscription** that gives you reliable access to popular open coding models.

Go works like any other provider in BotConnector. You subscribe to BotConnector Go and
...
BotConnector Go gives you access to these models for **$10/month**.

---

## How it works

BotConnector Go works like any other provider in BotConnector.

1. You sign in to **<a href={console}>BotConnector Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `BotConnector Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to BotConnector Go.
:::
...
BotConnector Go is designed for [BotConnector](https://opencode.ai) and other coding agents
...
Besides BotConnector, the following clients have been validated to work properly
with BotConnector Go. Although we do not guarantee that they will continue to work in the future.

| Client            | Session support                                                                                                                                                                                                                             |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Builds containing [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) send the header on main and auxiliary BotConnector requests. The fix was merged after v0.21.0; that release alone does not include it.                 |
...
| **Pi**            | Current builds send session information for BotConnector. Update older installations.                                                                                                                                                           |
| **jcode**         | Update to **v0.81.6 or later**, which includes the [session-header fix](https://github.com/1jehuang/jcode/issues/1167).                                                                                                                     |
| **Kilo Code CLI** | Builds containing [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restore BotConnector session headers. This fix covers the CLI, not the VS Code extension. See [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
4. Have **no lock-in** by allowing you to use any other provider with BotConnector as well.
