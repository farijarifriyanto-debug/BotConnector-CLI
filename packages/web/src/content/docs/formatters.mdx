---
title: Formatters
description: BotConnector uses language specific formatters.
---

BotConnector can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before BotConnector will run them.

---

## Built-in

BotConnector comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
When formatters are enabled, BotConnector will use `prettier` for matching files if your project has `prettier` in `package.json`.

---

## How it works

When BotConnector writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your BotConnector config.
...
```json title="botconnector.json" {5}
{
  "formatter": {
    "prettier": {
      "disabled": true
...
```json title="botconnector.json" {4-14}
{
  "formatter": {
    "prettier": {
