...
You can add external tools to BotConnector using the _Model Context Protocol_, or MCP. BotConnector supports both local and remote servers.
...
You can define MCP servers in your [BotConnector Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="botconnector.jsonc" {6}
{
  "mcp": {
    "name-of-mcp-server": {
      // ...
...
```jsonc title="botconnector.jsonc" {15}
{
  "mcp": {
    "my-local-mcp-server": {
      "type": "local",
...
BotConnector automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, BotConnector will:
...
If the server requires authentication, BotConnector will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
```json title="botconnector.json" {7-11}
{
  "mcp": {
    "my-oauth-server": {
      "type": "remote",
...
The `mcp auth` command will open your browser for authorization. After you authorize, BotConnector will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in BotConnector, alongside built-in tools. So you can manage them through the BotConnector config like any other tool.
...
```json title="botconnector.json" {14}
{
  "mcp": {
    "my-mcp-foo": {
      "type": "local",
...
```json title="botconnector.json" {11, 14-18}
{
  "mcp": {
    "my-mcp": {
      "type": "local",
...
```json title="botconnector.json" {4-8}
{
  "mcp": {
    "sentry": {
      "type": "remote",
...
This will open a browser window to complete the OAuth flow and connect BotConnector to your Sentry account.
...
```json title="botconnector.json" {4-7}
{
  "mcp": {
    "context7": {
      "type": "remote",
...
```json title="botconnector.json" {7-9}
{
  "mcp": {
    "context7": {
      "type": "remote",
