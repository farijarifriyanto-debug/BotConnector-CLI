...
To debug issues with BotConnector, start by checking the logs and local data it stores on disk.
...
To uninstall the BotConnector CLI and remove its related files, run:
...
To remove BotConnector Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

BotConnector Desktop runs a local BotConnector server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `BotConnector` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/botconnector/botconnector.jsonc` (or `~/.config/botconnector/botconnector.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/botconnector.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\botconnector.jsonc`
...
BotConnector can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/botconnector/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.botconnector/plugins/`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so BotConnector can rebuild it.

1. Quit BotConnector Desktop completely.
...
3. Restart BotConnector Desktop.

---

### Fix server connection issues

BotConnector Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `botconnector.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, BotConnector Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with BotConnector's features.

---

### Notifications not showing

BotConnector Desktop only shows system notifications when:

- notifications are enabled for BotConnector in your OS settings, and
...
1. Quit BotConnector Desktop.
2. Find and delete these files (they live in the BotConnector Desktop app data directory):
...
If you're experiencing issues with BotConnector:
