---
title: Providers
description: Using any LLM provider in BotConnector.
---

import config from "../../../config.mjs"
export const console = config.console

BotConnector uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your BotConnector config.
...
You can customize the providers through the `provider` section in your BotConnector
config.
...
```json title="botconnector.json" {6}
{
  "provider": {
    "anthropic": {
      "options": {
...
BotConnector Zen is a list of models provided by the BotConnector team that have been
tested and verified to work well with BotConnector. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with BotConnector Zen.
:::

1. Run the `/connect` command in the TUI, select `BotConnector Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in BotConnector and is completely optional to use.

---

## BotConnector Go

BotConnector Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the BotConnector team that have been
tested and verified to work well with BotConnector.

1. Run the `/connect` command in the TUI, select `BotConnector Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with BotConnector:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `botconnector.json`:

   ```json title="botconnector.json"
   {
     "provider": {
       "amazon-bedrock": {
         "options": {
...
BotConnector. Anthropic explicitly prohibits this.

Previous versions of BotConnector came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in BotConnector with zero setup:
...
```json title="botconnector.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "provider": {
    "atomic-chat": {
...
BotConnector does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. BotConnector refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
BotConnector supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; BotConnector uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize BotConnector. Sign in and approve.

   :::note
   BotConnector requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, BotConnector surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — BotConnector forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in BotConnector is experimental. Features, configuration, and
behavior may change in future releases.
:::

BotConnector integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `BotConnector`, Scopes: `api`
...
BotConnector uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock BotConnector
to only use your own GitLab-hosted instance, add the following to your
`botconnector.json` file. It is also recommended to disable session sharing.
...
Customize through `botconnector.json`:
...
When a `duo-workflow-*` model is selected, BotConnector will:
...
4. Route tool execution requests through BotConnector's permission-gated tool system
...
To use GMI Cloud with BotConnector:
...
To use Google Vertex AI with BotConnector:
...
```jsonc title="~/.config/botconnector/botconnector.jsonc"
{
  "provider": {
    "helicone": {
      "npm": "@ai-sdk/openai-compatible",
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each BotConnector conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each BotConnector conversation listed as a separate session.
...
```json title="botconnector.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "provider": {
    "llama.cpp": {
      "npm": "@ai-sdk/openai-compatible",
...
```json title="botconnector.json" "lmstudio" {5, 6, 8, 10-14}
{
  "provider": {
    "lmstudio": {
      "npm": "@ai-sdk/openai-compatible",
...
:::tip
Ollama can automatically configure itself for BotConnector. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="botconnector.json" "ollama" {5, 6, 8, 10-14}
{
  "provider": {
    "ollama": {
      "npm": "@ai-sdk/openai-compatible",
...
To use Ollama Cloud with BotConnector:
...
3. Copy the API key for use in BotConnector.
...
6. **Important**: Before using cloud models in BotConnector, you must pull the model information locally:
...
BotConnector Zen is a list of tested and verified models provided by the BotConnector team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>BotConnector Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **BotConnector Zen**.

   ```txt
   /connect
   ```

3. Enter your BotConnector API key.
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with BotConnector:
...
5. Complete sign-in in the browser that opens. BotConnector captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
The model catalog is provided automatically. A minimal `botconnector.json` is all that's needed:
...
2. Select **SuperGrok Subscription**. BotConnector opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by BotConnector. BotConnector polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
BotConnector refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in botconnector.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `botconnector.json` file in your project directory:

   ```json title="botconnector.json" ""myprovider"" {5-15}
   {
     "provider": {
       "myprovider": {
         "npm": "@ai-sdk/openai-compatible",
...
```json title="botconnector.json" {9,11,17-20}
{
  "provider": {
    "myprovider": {
      "npm": "@ai-sdk/openai-compatible",
...
The `limit` fields allow BotConnector to understand how much context you have left. Standard providers pull these from models.dev automatically.
