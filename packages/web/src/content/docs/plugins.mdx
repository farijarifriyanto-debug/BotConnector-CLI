---
title: Plugins
description: Write your own plugins to extend BotConnector.
---

Plugins allow you to extend BotConnector by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify BotConnector's default behavior.
...
- `.botconnector/plugins/` - Project-level plugins
- `~/.config/botconnector/plugins/` - Global plugins
...
1. Global config (`~/.config/botconnector/botconnector.json`)
2. Project config (`botconnector.json`)
3. Global plugin directory (`~/.config/botconnector/plugins/`)
4. Project plugin directory (`.botconnector/plugins/`)
...
```json title=".botconnector/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
BotConnector runs `bun install` at startup to install these. Your plugins and tools can then import them.

```ts title=".botconnector/plugins/my-plugin.ts"
...
```js title=".botconnector/plugins/example.js"
...
```js title=".botconnector/plugins/notification.js"
...
:::note
If you’re using the BotConnector desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
```javascript title=".botconnector/plugins/env-protection.js"
...
```javascript title=".botconnector/plugins/inject-env.js"
...
```ts title=".botconnector/plugins/custom-tools.ts"
...
```ts title=".botconnector/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".botconnector/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
