---
title: Config
description: Using the BotConnector JSON config.
---

You can configure BotConnector using a JSON config file.

---

## Format

BotConnector supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="botconnector.jsonc"
{
...
2. **Global config** (`~/.config/botconnector/botconnector.json`) - user preferences
3. **Custom config** (`BOTCONNECTOR_CONFIG` env var) - custom overrides
4. **Project config** (`botconnector.json` in project) - project-specific settings
5. **`.botconnector` directories** - agents, commands, plugins
6. **Inline config** (`BOTCONNECTOR_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/botconnector/` on macOS) - admin-controlled
...
:::note
The `.botconnector` and `~/.config/botconnector` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global BotConnector config in `~/.config/botconnector/botconnector.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/botconnector/tui.json`.
...
Add `botconnector.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When BotConnector starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `BOTCONNECTOR_CONFIG` environment variable.

```bash
export BOTCONNECTOR_CONFIG=/path/to/my/custom-config.json
...
Specify a custom config directory using the `BOTCONNECTOR_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.botconnector` directory, and should
follow the same structure.

```bash
export BOTCONNECTOR_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```

The custom directory is loaded after the global config and `.botconnector` directories, so it **can override** their settings.
...
Drop an `botconnector.json` or `botconnector.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/botconnector/` |
| Linux    | `/etc/botconnector/`                         |
| Windows  | `%ProgramData%\botconnector`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, BotConnector reads managed preferences from the `ai.botconnector.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

BotConnector checks these paths:

1. `/Library/Managed Preferences/<user>/ai.botconnector.managed.plist`
2. `/Library/Managed Preferences/ai.botconnector.managed.plist`

The plist keys map directly to `botconnector.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.botconnector.managed` PayloadType. The BotConnector config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.botconnector.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.botconnector.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.botconnector</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
BotConnector validates server/runtime and TUI configuration with bundled schemas. A BotConnector-owned public schema URL is not published yet.



Runtime validation remains available; editor autocomplete will use a BotConnector-owned schema once published.
...
Legacy `theme`, `keybinds`, and `tui` keys in `botconnector.json` are deprecated and automatically migrated when possible.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your BotConnector server.
...
If not specified, BotConnector will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your BotConnector config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, BotConnector tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny BotConnector actions on configured resources. Currently, policies can control which providers BotConnector may use.
...
BotConnector normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, BotConnector omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/botconnector/agents/` or `.botconnector/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, BotConnector will fall back to `"build"` with a warning.
...
You can also define commands using markdown files in `~/.config/botconnector/commands/` or `.botconnector/commands/`. [Learn more here](/docs/commands).
...
BotConnector uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
BotConnector will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend BotConnector with custom tools, hooks, and integrations.

Place plugin files in `.botconnector/plugins/` or `~/.config/botconnector/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict BotConnector to only use specific providers rather than disabling them one by one.
