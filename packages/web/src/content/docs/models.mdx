...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start BotConnector.
...
Here are several models that work well with BotConnector, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
```json title="botconnector.json" {3}
{
  "model": "lmstudio/google/gemma-3n-e4b"
}
```

Here the full ID is `provider_id/model_id`. For example, if you're using [BotConnector Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="botconnector.jsonc" {7-12,19-24}
{
  "provider": {
    "openai": {
      "models": {
...
```jsonc title="botconnector.jsonc" {6-21}
{
  "provider": {
    "opencode": {
      "models": {
...
Many models support multiple variants with different configurations. BotConnector ships with built-in default variants for popular providers.

### Built-in variants

BotConnector ships with default variants for many providers:
...
```jsonc title="botconnector.jsonc" {7-18}
{
  "provider": {
    "openai": {
      "models": {
...
When BotConnector starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the BotConnector config.
