...
- Locally by placing them in the `.botconnector/tools/` directory of your project.
- Or globally, by placing them in `~/.config/botconnector/tools/`.
...
```ts title=".botconnector/tools/database.ts" {1}
...
```ts title=".botconnector/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".botconnector/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".botconnector/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".botconnector/tools/add.py"
import sys
...
```ts title=".botconnector/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".botconnector/tools/add.py")
