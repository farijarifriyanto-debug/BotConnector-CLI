...
import { InstructionContext } from "@botconnector/core/instruction-context"
...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.BOTCONNECTOR_DISABLE_PROJECT_CONFIG
      let scanned = false
      process.env.BOTCONNECTOR_DISABLE_PROJECT_CONFIG = "1"
      ...
      yield* SystemContextRegistry.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.BOTCONNECTOR_DISABLE_PROJECT_CONFIG
            else process.env.BOTCONNECTOR_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
