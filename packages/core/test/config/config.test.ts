...
import { ConfigProvider } from "@botconnector/core/config/provider"
...
import { ConfigMigrateV1 } from "@botconnector/core/v1/config/migrate"
...
import { Policy } from "@botconnector/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads BotConnector JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "botconnector.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "botconnector.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "botconnector.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "botconnector.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
  ...
  it.live("accepts $schema metadata without writing it into config files", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "botconnector.json")
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "botconnector.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "botconnector.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "botconnector.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .botconnector configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".botconnector"), { recursive: true })
            await fs.mkdir(path.join(directory, ".botconnector"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "botconnector.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "botconnector.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "botconnector.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "botconnector.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "botconnector.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".botconnector", "botconnector.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".botconnector", "botconnector.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".botconnector")),
              AbsolutePath.make(path.join(directory, ".botconnector")),
            ])
