...
// etc.).
//
// The visual language uses BotConnector's local OAuth styling: the design tokens are a curated
// a compact subset of local semantic tokens in `packages/ui/src/styles/theme.css`, and
// the brand mark is rendered locally without external assets.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `BotConnector is now connected to ${escapeHtml(provider)}.` : "BotConnector is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `BotConnector couldn't finish connecting to ${escapeHtml(provider)}.`
        : "BotConnector couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from BotConnector.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · BotConnector</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("BotConnector couldn't finish connecting to "+PROVIDER+"."):"BotConnector couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from BotConnector."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("BotConnector is now connected to "+PROVIDER+"."):"BotConnector is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const WORDMARK = `<div class="wordmark" aria-label="BotConnector" role="img">BotConnector</div>`
