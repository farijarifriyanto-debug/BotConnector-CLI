...
import { Permission } from "@botconnector/schema/permission"
...
export class Service extends Context.Service<Service, Interface>()("@botconnector/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["botconnector.json", "botconnector.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".botconnector", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".botconnector")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".botconnector").toReversed()
    ...
    // global config, project files, then `.botconnector` files.
