...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("botconnector.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}botconnector.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/botconnector/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.botconnector/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.botconnector/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.botconnector/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.botconnector/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent BotConnector from reading sensitive files",
  "Use {highlight}botconnector run{/highlight} for non-interactive scripting",
  "Use {highlight}botconnector --continue{/highlight} to resume the last session",
  "Use {highlight}botconnector run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}botconnector serve{/highlight} for headless API access to BotConnector",
  "Use {highlight}botconnector run --attach{/highlight} to connect to a running server",
  "Run {highlight}botconnector upgrade{/highlight} to update to the latest version",
  "Run {highlight}botconnector auth list{/highlight} to see all configured providers",
  "Run {highlight}botconnector agent create{/highlight} for guided agent creation",
  ...
  "Run {highlight}botconnector debug config{/highlight} to troubleshoot configuration",
  ...
  "Use the BotConnector npm package for supported CLI installs",
  "Use {highlight}/connect{/highlight} to configure an available BotConnector model provider",
