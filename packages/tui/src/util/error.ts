...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `botconnector models` to list available models",
      "Or check your BotConnector config provider/model names",
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`botconnector auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, BotConnector does not support MCP authentication yet.`
  }
  return undefined
}
