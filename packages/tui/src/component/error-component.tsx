...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // Safe fallback palette per mode (mirrors the BotConnector theme fallback) since the
      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>BotConnector {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the BotConnector crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n… (truncated)"
  const head = `The BotConnector TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
